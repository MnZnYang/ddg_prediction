...
class SE3Transformer(torch.nn.Module):
    def __init__(
        ...
        *,
        ...
        self.u_scale = torch.nn.Parameter(torch.tensor(0.3))
    ...
    def forward(self, data):
        ...
        feats = data["embedding"]
        if feats.dim() == 2:
            feats = feats.unsqueeze(dim=0)  # [1,L,1280]
        ...
        wt_idx = data["wt_idx"]
        if wt_idx.dim() == 2 and wt_idx.size(0) == 1:
            wt_idx = wt_idx.squeeze(0) # [L]

        edges = self.epistasis_edge_proj(epistasis_edges)

        seq0 = self.esm2_transform(feats)  # [B,L,320]

        if self.training:
            if torch.rand((), device=seq0.device) < self.p_seq_off:
                seq0 = torch.zeros_like(seq0)
        ...
        zeroshot_single_mut = zeroshot_single_mut.squeeze(0)  # [L,20]
        zeroshot_single_mut = (zeroshot_single_mut - zeroshot_single_mut.mean()) / (zeroshot_single_mut.std(unbiased=False) + 1e-8)
        single_mut = single_mut + self.beta * zeroshot_single_mut  # [L,20]
